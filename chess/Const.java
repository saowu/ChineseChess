...
public class Const {
    ...
    // 将:0x11 士:0x12 象:0x13 车: 0x14 马:0x15 炮:0x16 卒:0x17
    // 帅:0x01 仕:0x02 相:0x03 車: 0x04 馬:0x05 砲:0x06 兵:0x07
    public final static Map<String, Short> PIECE_MAP = new HashMap<>() {{
        // 黑
        put("将", (short) 0x11);
        put("士", (short) 0x12);
        put("象", (short) 0x13);
    ...
    public static void printPiece(short[][] posPiece, short flagPlayer) {
            ...
            for (int i = RANGE_CHESSBOARD[1][0]; i < RANGE_CHESSBOARD[1][1] + 1; ++i) {
                ...
                for (int j = RANGE_CHESSBOARD[0][0]; j < RANGE_CHESSBOARD[0][1] + 1; ++j) {
                    ...
                    if (Const.PIECE_MAP.containsKey(piece) && Const.PIECE_MAP.get(piece) < 10) {
                        System.out.print("\033[31m" + piece + "\033[0m ");
                        continue;
                    }
        ...
        System.out.println("-----------------------");
        System.out.println("九 八 七 六 五 四 三 二 一");
    }
}
